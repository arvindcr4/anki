...
- context-sensitive guidance and row actions so the right next step is one click away, including trend-aware restart prompts, consistency nudges, visually emphasized resume-last-capture shortcuts, current-gap recovery for both quiet days and quiet stretches, gap-length-aware import recovery, burst-review followups, and recovery imports
