...
- clickable browse-by-date actions, including date-labeled today that upgrades to burst-day review when today dominates the week, a last-7-days action that still shows the concrete visible range, a panel-level current-gap-week shortcut with visible gap length when relevant, date-bounded streak, date-labeled burst-day review, resume-your-last-capture, panel-level busiest-day recovery that upgrades to burst-day review when appropriate and remains visible when burst significance overlaps with the latest session, and summary-pill shortcuts
- dedicated daily cards panel styling, including a compact 7-day activity strip with visible dates, stateful highlighting for streak/latest/busiest/current-gap states, stateful Today-status pills, a decoding legend, context-aware capture hints that explain burst and gap emphasis, accessible labels, discoverability hints, and assistive-tech semantics
- burst-heavy weeks should be legible both in the summary pills and directly on the relevant row/bar, including a row-level Review burst day CTA on the dominant session
...
- card-creation affordances from the daily cards surface, including context-aware create/import actions with momentum-aware import wording, a zero-to-first-card prompt with import fallback, a capture-enabled Today bar, an empty-Today status pill that can shift between start-this-week, ready-to-capture, and ready-to-restart language, a resume-last-capture fallback on empty Today, and a keep-capturing shortcut for Today
- momentum cues such as today-status with cards-across-notes detail, active/quiet day balance, consistency, gap, latest-capture metadata with session size and overlap with the busiest session, visible range, pace, trend, density, burst share, streaks, busiest-day summaries that can also acknowledge the latest session when they coincide, and next-step guidance so the panel encourages continuity, not just navigation, with today-status also able to acknowledge when today is the dominant batch, latest and busiest pills sharing overlap styling when they refer to the same session, density remaining directly browseable when users want to inspect high-card-output weeks, gap summaries calling out the latest capture date, and multi-day lulls named consistently as the current gap with their own browse target and stronger long-gap emphasis
- context-sensitive guidance and row actions so the right next step is one click away, including trend-aware restart prompts, consistency nudges, visually emphasized resume-last-capture shortcuts, latest-row resume-last-capture CTAs, current-gap recovery for empty Today as well as quiet days and quiet stretches, row-level resume-last-capture recovery, row-level week-context fallback, date-anchored current-gap guidance, gap-length-aware import recovery, busiest-row import followups, burst-review followups, burst-today import followups, recovery imports, and stateful guidance styling
