...
- momentum cues such as active/quiet day balance, consistency, gap, latest-capture metadata with session size, visible range, pace, trend, density, burst share, streaks, busiest-day summaries, and next-step guidance so the panel encourages continuity, not just navigation, with density remaining directly browseable when users want to inspect high-card-output weeks, gap summaries calling out the latest capture date, and multi-day lulls named consistently as the current gap with their own browse target
