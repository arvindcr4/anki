...
- momentum cues such as today-status, active/quiet day balance, consistency, gap, latest-capture metadata with session size and overlap with the busiest session, visible range, pace, trend, density, burst share, streaks, busiest-day summaries that can also acknowledge the latest session when they coincide, and next-step guidance so the panel encourages continuity, not just navigation, with today-status also able to acknowledge when today is the dominant batch, density remaining directly browseable when users want to inspect high-card-output weeks, gap summaries calling out the latest capture date, and multi-day lulls named consistently as the current gap with their own browse target
- context-sensitive guidance and row actions so the right next step is one click away, including trend-aware restart prompts, consistency nudges, visually emphasized resume-last-capture shortcuts, latest-row resume-last-capture CTAs, current-gap recovery for both quiet days and quiet stretches, row-level resume-last-capture recovery, row-level week-context fallback, date-anchored current-gap guidance, gap-length-aware import recovery, busiest-row import followups, burst-review followups, burst-today import followups, recovery imports, and stateful guidance styling
