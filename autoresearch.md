...
- context-sensitive guidance and row actions so the right next step is one click away, including trend-aware restart prompts, consistency nudges, visually emphasized resume-last-capture shortcuts, quiet-stretch restart actions with a clear current-gap marker, gap-length-aware import recovery, burst-review followups, and recovery imports
