#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")"
python3 - <<'PY'
from pathlib import Path
import py_compile

py_compile.compile('qt/aqt/deckbrowser.py', doraise=True)
texts = []
for candidate in [
    Path('qt/aqt/deckbrowser.py'),
    Path('qt/aqt/data/web/css/deckbrowser.scss'),
    Path('docs/daily-deck-ux.md'),
]:
    if candidate.exists():
        texts.append(candidate.read_text())
joined = '\n'.join(texts)
score = 0
timeline_surface = 0
browse_by_date = 0
visual_hierarchy = 0
capture_support = 0
query_efficiency = 0
checks = [
    ('DailyCardsGroup', 3, 'timeline'),
    ('daily-cards-panel', 2, 'timeline'),
    ('daily-cards-heatmap', 2, 'visual'),
    ('7 day activity strip', 1, 'visual'),
    ('daily-cards-bar', 2, 'visual'),
    ('is-streak-bar', 1, 'visual'),
    ('is-latest-bar', 1, 'visual'),
    ('is-busiest-bar', 1, 'visual'),
    ('is-burst-bar', 1, 'visual'),
    ('is-gap-bar', 1, 'visual'),
    ('daily-cards-bar-label', 1, 'visual'),
    ('daily-cards-bar-date', 1, 'visual'),
    ('daily-cards-bar-count', 1, 'visual'),
    ('is-capture-bar', 1, 'visual'),
    ("Create today's first card from the activity strip", 2, 'capture'),
    ('aria-label', 2, 'visual'),
    ('cards across', 2, 'timeline'),
    ('daily-cards-row', 2, 'visual'),
    ('browseAdded', 3, 'browse'),
    ('browseRecent', 2, 'browse'),
    ('browseStreak', 2, 'browse'),
    ('added:', 3, 'browse'),
    ('No cards added', 1, 'timeline'),
    ('daily-cards-empty-summary', 1, 'visual'),
    ('is-empty-cluster', 1, 'visual'),
    ('Quiet stretch', 2, 'timeline'),
    ('No cards added on', 2, 'timeline'),
    ('Restart after quiet stretch', 2, 'capture'),
    ('daily-cards-quiet-restart', 1, 'visual'),
    ('daily-cards-quiet-day-restart', 1, 'visual'),
    ('daily-cards-quiet-resume', 1, 'visual'),
    ('Restart after quiet day', 2, 'capture'),
    ('daily-cards-quiet-import', 1, 'visual'),
    ('Current gap', 2, 'timeline'),
    ('is-current-gap', 1, 'visual'),
    ('is-long-gap', 1, 'visual'),
    ('longer break', 1, 'timeline'),
    ('current gap since', 1, 'timeline'),
    ('resume the timeline', 1, 'timeline'),
    ('gap is already several days long', 1, 'timeline'),
    ('Browse week context', 1, 'browse'),
    ('0 cards / 0 notes', 1, 'timeline'),
    ('deck-browser-secondary-row', 1, 'visual'),
    ('Daily cards', 1, 'docs'),
    ('date-oriented', 1, 'docs'),
    ('daily-cards-summary', 2, 'visual'),
    ('daily-cards-rollover', 2, 'visual'),
    ('daily-cards-meta', 2, 'visual'),
    ('daily-cards-pill', 1, 'visual'),
    ('daily-cards-summary-label', 1, 'visual'),
    ('daily-cards-summary-counts', 1, 'visual'),
    ('is-empty', 1, 'visual'),
    ('_count_label', 2, 'visual'),
    ('grammatically correct', 1, 'timeline'),
    ('Day resets at', 1, 'timeline'),
    ("Add cards today and they'll appear here", 2, 'timeline'),
    ('changes.note', 1, 'timeline'),
    ('changes.card', 1, 'timeline'),
    ('Browse cards', 1, 'browse'),
    ('Browse cards →', 2, 'browse'),
    ('rows="\n".join(rows)', 2, 'visual'),
    ('Browse last 7 days', 2, 'browse'),
    ('Browse visible week (', 1, 'browse'),
    ('Browse today', 2, 'browse'),
    ('Browse today (', 1, 'browse'),
    ('Browse latest day', 2, 'browse'),
    ('Browse latest day (', 1, 'browse'),
    ('Browse current gap week', 2, 'browse'),
    ('Browse current gap week (', 1, 'browse'),
    ('day gap', 1, 'timeline'),
    ('Current gap pill browse', 1, 'browse'),
    ('Resume last capture', 2, 'browse'),
    ('Resume last capture (', 1, 'browse'),
    ('Resume last capture →', 2, 'browse'),
    ('latest_active_group', 1, 'timeline'),
    ('Cards added on', 1, 'browse'),
    ('Cards added in last 7 days', 1, 'browse'),
    ('scheduler cutoff', 2, 'timeline'),
    ('unique notes across the full range', 1, 'timeline'),
    ('Last 7 days:', 1, 'visual'),
    ('Create today\'s first card', 2, 'capture'),
    ('Create cards', 2, 'capture'),
    ('Create first card', 2, 'capture'),
    ('Restart today', 2, 'capture'),
    ('Create another', 2, 'capture'),
    ('Import cards', 2, 'capture'),
    ('Import more', 2, 'capture'),
    ('Import to rebuild momentum', 2, 'capture'),
    ('Import more cards', 1, 'capture'),
    ('addcards', 2, 'capture'),
    ('importcards', 2, 'capture'),
    ('daily-cards-create', 1, 'capture'),
    ('daily-cards-import', 1, 'capture'),
    ('daily-cards-resume', 1, 'visual'),
    ('daily-cards-resume-row-action', 1, 'visual'),
    ('daily-cards-busiest-shortcut', 1, 'visual'),
    ('daily-cards-burst-shortcut', 1, 'visual'),
    ('daily-cards-current-gap-shortcut', 1, 'visual'),
    ('Most active', 1, 'visual'),
    ('Latest session', 2, 'visual'),
    ('Today should not erase', 1, 'timeline'),
    ('Current gap', 2, 'timeline'),
    ('daily-cards-statuses', 1, 'visual'),
    ('status_badges', 1, 'visual'),
    ('is-latest-session', 1, 'visual'),
    ('daily-cards-status', 1, 'visual'),
    ('daily-cards-status-secondary', 1, 'visual'),
    ('daily-cards-status-burst', 1, 'visual'),
    ('Burst session', 1, 'visual'),
    ('Today:', 2, 'timeline'),
    ('across note', 1, 'timeline'),
    ('most active)', 1, 'timeline'),
    ('ready to capture', 2, 'timeline'),
    ('ready to restart', 2, 'timeline'),
    ("start this week's timeline", 2, 'timeline'),
    ('active day', 2, 'timeline'),
    ('daily-cards-today', 1, 'visual'),
    ('has-today-cards', 1, 'visual'),
    ('is-most-active-today', 1, 'visual'),
    ('is-ready-to-capture', 1, 'visual'),
    ('is-ready-to-restart', 1, 'visual'),
    ('is-start-this-week', 1, 'visual'),
    ('daily-cards-gap', 1, 'visual'),
    ('daily-cards-latest', 1, 'visual'),
    ('is-most-active-latest', 1, 'visual'),
    ('is-latest-busiest', 1, 'visual'),
    ('daily-cards-quiet', 1, 'visual'),
    ('daily-cards-consistency', 1, 'visual'),
    ('Quiet days:', 2, 'timeline'),
    ('Consistency:', 2, 'timeline'),
    ('% active', 2, 'timeline'),
    ('Browse weekly consistency context', 2, 'browse'),
    ('Browse latest capture', 2, 'browse'),
    ('Browse active week', 2, 'browse'),
    ('Browse quiet week context', 2, 'browse'),
    ('Browse visible week', 2, 'browse'),
    ('Gap:', 2, 'timeline'),
    ('Current gap:', 2, 'timeline'),
    ('Latest:', 2, 'timeline'),
    ('most active)', 1, 'timeline'),
    ("latest session’s card and note counts", 1, 'timeline'),
    ('captured today', 1, 'timeline'),
    ('last capture', 1, 'timeline'),
    ('last capture on', 1, 'timeline'),
    ('is-current-gap', 1, 'visual'),
    ('daily-cards-range', 1, 'visual'),
    ('Range:', 2, 'timeline'),
    ('daily-cards-pace', 1, 'visual'),
    ('Pace:', 2, 'timeline'),
    ('cards/active day', 2, 'timeline'),
    ('Browse weekly pace context', 2, 'browse'),
    ('daily-cards-trend', 1, 'visual'),
    ('Trend:', 2, 'timeline'),
    ('Trend: rising', 1, 'timeline'),
    ('Trend: cooling', 1, 'timeline'),
    ('Trend: steady', 1, 'timeline'),
    ('Trend: just started', 1, 'timeline'),
    ('Browse weekly trend context', 2, 'browse'),
    ('daily-cards-burst', 1, 'visual'),
    ('Burst:', 2, 'timeline'),
    ('% on busiest day', 2, 'timeline'),
    ('Browse burst day', 2, 'browse'),
    ('Browse burst day (', 1, 'browse'),
    ('Review burst day', 2, 'browse'),
    ('Review burst day (', 1, 'browse'),
    ('Review burst day →', 2, 'browse'),
    ('daily-cards-burst-row-action', 1, 'visual'),
    ('daily-cards-burst-import', 1, 'visual'),
    ('daily-cards-density', 1, 'visual'),
    ('Density:', 2, 'timeline'),
    ('cards/note', 2, 'timeline'),
    ('Browse weekly density context', 2, 'browse'),
    ('Current streak', 2, 'timeline'),
    ('Last streak', 2, 'timeline'),
    ('daily-cards-streak', 1, 'visual'),
    ('Browse current streak (', 1, 'browse'),
    ('Browse last streak (', 1, 'browse'),
    ('daily-cards-insight', 2, 'timeline'),
    ('daily-cards-insight-link', 1, 'visual'),
    ('Review burst-heavy week', 2, 'browse'),
    ('Browse recent trend', 2, 'browse'),
    ('is-burst', 1, 'visual'),
    ('is-rising', 1, 'visual'),
    ('is-starting', 1, 'visual'),
    ('is-cooling', 1, 'visual'),
    ('is-gap', 1, 'visual'),
    ('is-gap-burst', 1, 'visual'),
    ('is-consistent', 1, 'visual'),
    ('is-spread', 1, 'visual'),
    ('Insight:', 2, 'timeline'),
    ('one big capture session', 1, 'timeline'),
    ('recent capture is accelerating', 1, 'timeline'),
    ('recent capture has cooled', 1, 'timeline'),
    ('capture has paused since', 2, 'timeline'),
    ('one big capture session was followed by a current gap', 2, 'timeline'),
    ("week's capture just started", 2, 'timeline'),
    ('capture has been consistent', 1, 'timeline'),
    ('daily-cards-guidance', 2, 'timeline'),
    ('daily-cards-guidance-block', 1, 'visual'),
    ('daily-cards-guidance-block.is-gap', 1, 'visual'),
    ('daily-cards-guidance-block.is-burst', 1, 'visual'),
    ('daily-cards-guidance-block.is-starting', 1, 'visual'),
    ('daily-cards-guidance-block.is-consistent', 1, 'visual'),
    ('role="status"', 1, 'visual'),
    ('aria-live="polite"', 1, 'visual'),
    ('daily-cards-guidance-actions', 2, 'capture'),
    ('daily-cards-strip-hint', 1, 'visual'),
    ('daily-cards-strip-legend', 1, 'visual'),
    ('daily-cards-legend-item', 1, 'visual'),
    ('daily-cards-legend-swatch', 1, 'visual'),
    ('Create here', 1, 'capture'),
    ('is-capture', 1, 'visual'),
    ('is-gap', 1, 'visual'),
    ('is-burst', 1, 'visual'),
    ('Streak run', 1, 'timeline'),
    ('Tap a bar to browse that day', 2, 'timeline'),
    ('dashed empty bars mark the current gap', 2, 'timeline'),
    ('highlighted burst bar marks the busiest session', 2, 'timeline'),
    ('Bars light up as you create or import cards', 2, 'timeline'),
    ('Keep capturing while the topic is fresh', 2, 'timeline'),
    ('Keep the streak going', 2, 'capture'),
    ('restart the streak', 2, 'timeline'),
    ('Restart streak today', 2, 'capture'),
    ('Keep consistency going', 2, 'capture'),
    ('Extend rising trend', 2, 'capture'),
    ('Reverse cooling trend', 2, 'capture'),
    ('Import to rebuild momentum', 2, 'capture'),
    ('Build a streak today', 2, 'capture'),
    ('start this week\'s timeline', 2, 'timeline'),
    ('Busiest:', 2, 'timeline'),
    ('latest session)', 1, 'timeline'),
    ('Browse current streak', 2, 'browse'),
    ('Browse last streak', 2, 'browse'),
    ('Browse busiest day', 2, 'browse'),
    ('Create first card', 2, 'capture'),
    ('Create another', 2, 'capture'),
    ('Create first card →', 1, 'capture'),
    ('daily-cards-empty-resume', 1, 'visual'),
    ('Import cards', 2, 'capture'),
    ('Create today', 2, 'capture'),
    ('daily-cards-action-stack', 1, 'capture'),
    ('daily-cards-secondary-link', 1, 'visual'),
    ('is-capture-target', 1, 'visual'),
    ('count(distinct nid)', 2, 'query'),
    ('with recent_cards as', 2, 'query'),
    ('summary as', 1, 'query'),
    ('group by days_ago', 1, 'query'),
]
for needle, pts, bucket in checks:
    if needle in joined:
        score += pts
        if bucket == 'timeline':
            timeline_surface += 1
        elif bucket == 'browse':
            browse_by_date += 1
        elif bucket == 'visual':
            visual_hierarchy += 1
        elif bucket == 'docs':
            visual_hierarchy += 1
        elif bucket == 'capture':
            capture_support += 1
        elif bucket == 'query':
            query_efficiency += 1
print(f'METRIC daily_cards_ux_score={score}')
print('METRIC syntax_ok=1')
print(f'METRIC timeline_surface={timeline_surface}')
print(f'METRIC browse_by_date={browse_by_date}')
print(f'METRIC visual_hierarchy={visual_hierarchy}')
print(f'METRIC capture_support={capture_support}')
print(f'METRIC query_efficiency={query_efficiency}')
PY
